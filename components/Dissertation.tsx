    ...
    <section id="dissertation" ref={sectionRef} className="section-padding relative overflow-hidden">
      ...
      <div ref={root} className="section-container relative">
        ...
        <div className="max-w-3xl mb-12">
          ...
          <p className="diss-subtitle text-lg md:text-xl text-(--muted-foreground) mb-3 opacity-0">
            Third Year Project Report &mdash; B.Sc. (Hons) Computer Science
          </p>
          <p className="diss-author font-mono text-sm text-(--accent) opacity-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-12">
          {HIGHLIGHTS.map((item, i) => (
            <div
              key={item.title}
              className="diss-highlight-card glass-card p-5 relative group opacity-0 hover:border-(--accent)/30 transition-colors"
            >
              {/* Orbit decoration */}
              <div className="diss-orbit absolute -top-2 -right-2 w-8 h-8" style={{ transformOrigin: "center" }}>
                <div className="absolute top-0 left-1/2 w-1.5 h-1.5 rounded-full bg-(--accent) opacity-40" />
              </div>
              ...
              <h3 className="text-lg font-display font-bold text-(--foreground) mb-1 group-hover:text-(--accent) transition-colors">
                {item.title}
              </h3>
        ...
        <div className="diss-cta flex flex-col sm:flex-row items-start gap-4 opacity-0">
          <a
            ...
            className="group relative inline-flex items-center gap-3 px-8 py-3.5 font-mono text-sm uppercase tracking-wider bg-(--accent) text-(--background) rounded-lg overflow-hidden"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 px-8 py-3.5 font-mono text-sm uppercase tracking-wider border border-(--border) text-(--foreground) rounded-lg hover:border-(--accent) transition-colors duration-300"
          >
